/* src/components/simulator/index.tsx */
import React, { useState } from 'react';
import { Box } from '@mui/material';
import ProductTabs from './ProductTabs';
import CommandExecutor from './CommandExecutor';

import G2405 from './products/G2405';
import G2529 from './products/G2529';
import G2538 from './products/G2538';

const productMap = {
  G2405,
  G2538,
  G2529
};

const SimulatePanel = () => {
  const [productKey, setProductKey] = useState<'G2405' | 'G2538' | 'G2529'>('G2405');

  return (
    <Box>
      <ProductTabs
        products={Object.keys(productMap)}
        selected={productKey}
        onChange={(key) => setProductKey(key as keyof typeof productMap)}
      />
      <CommandExecutor commands={productMap[productKey]} />
    </Box>
  );
};

export default SimulatePanel;
